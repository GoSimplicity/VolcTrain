...
// 表单数据
const formData = reactive<UpdateWorkspaceRequest>({
  id: '',
  name: '',
  description: '',
  resourceQuota: {
...
// 填充表单数据
const fillForm = (workspace: Workspace) => {
  formData.id = workspace.id;
  formData.name = workspace.name;
  formData.description = workspace.description || '';
  formData.resourceQuota = { ...workspace.resourceQuota };
  formData.config = { ...workspace.config };
